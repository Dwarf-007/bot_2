#!/usr/bin/env python3
"""Run the G1 CampaignStateTransition runtime wiring smoke."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run G1 CampaignStateTransition runtime wiring smoke.")
    parser.add_argument(
        "--raw-root",
        default="",
...
def main() -> int:
    args = parse_args()
    smoke = CampaignStateTransitionRuntimeWiringSmoke(project_root=PROJECT_ROOT)
    ...
    if args.json_out:
        ...
        if not args.quiet:
            print(f"CampaignStateTransition runtime wiring smoke JSON written: {out}")
