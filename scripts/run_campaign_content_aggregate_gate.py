#!/usr/bin/env python3
"""Run the F3 CampaignContent aggregate gate."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run F3 CampaignContent aggregate gate.")
    parser.add_argument(
        "--raw-root",
        default="",
...
def main() -> int:
    args = parse_args()
    gate = CampaignContentAggregateGate(project_root=PROJECT_ROOT)
    ...
    if args.json_out:
        ...
        if not args.quiet:
            print(f"CampaignContent F3 aggregate JSON written: {out}")
