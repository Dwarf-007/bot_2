#!/usr/bin/env python3
"""Run the F2 CharacterCreationAdvisor aggregate gate."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run F2 CharacterCreationAdvisor aggregate gate.")
    parser.add_argument(
        "--raw-root",
        default="",
...
def main() -> int:
    args = parse_args()
    gate = CharacterCreationAggregateGate(project_root=PROJECT_ROOT)
    ...
    if args.json_out:
        ...
        if not args.quiet:
            print(f"CharacterCreation F2 aggregate JSON written: {out}")
