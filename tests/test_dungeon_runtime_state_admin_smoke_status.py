...
def test_smoke_status_is_compact(tmp_path: Path):
    b = bundle(tmp_path)
    ...
    svc.reset(campaign_id="c1", channel_id="ch1", player_id="p1")
    result = svc.smoke_status(campaign_id="c1", channel_id="ch1")
    assert result["ok"] is True
    assert result["visibility_available"] is True
    assert result["authoritative_exists"] is True
